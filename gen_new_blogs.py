#!/usr/bin/env python3
"""生成 SKILL_TESTING_BLOG 和 AGENT_TESTING_BLOG 两篇博客 HTML"""
...
# 1. Skill 测试实战
with open(r'E:\project\agent_test\SKILL_TESTING_BLOG.md', 'r', encoding='utf-8') as f:
    md1 = f.read()
convert_md_to_blog_html(
    md1,
    title='Skill 测试实战：从脚本到 Agent 的分层验收方法',
    category='AI 测试',
    date='2026年5月13日',
    filename='skill-testing-blog.html'
)

# 2. Agent 测试笔记
with open(r'E:\project\agent_test\AGENT_TESTING_BLOG.md', 'r', encoding='utf-8') as f:
    md2 = f.read()
convert_md_to_blog_html(
    md2,
    title='LangGraph Agent 测试笔记：视频创作图的状态与分支验收',
    category='AI 测试',
    date='2026年5月13日',
    filename='agent-testing-blog.html'
)
